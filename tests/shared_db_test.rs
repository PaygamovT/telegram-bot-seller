...
async fn test_db_initialization_and_migrations() {
    ...
    assert!(
        db_path.exists(),
        "SQLite database file was not created at expected path"
    );
    ...
    let contact_res: (i64, String, String) = conn
        .interact(|conn| {
            let mut stmt = conn
                .prepare("SELECT chat_id, first_name, username FROM contacts WHERE chat_id = ?")
                .unwrap();
    ...
    assert!(
        fk_fail_res.is_err(),
        "Foreign key constraint on order_items should have failed"
    );
